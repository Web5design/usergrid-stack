...
//      assertEquals("User Account Activated", account_activation_message.getSubject());
  ...
  private Message[] getMessages(String host, String user, String password) 
    ...
    for (Message m : msgs) {
      ...
      logger.info("Body content 0 " 
              + (String) ((MimeMultipart) m.getContent()).getBodyPart(0).getContent());
      logger.info("Body content 1 " 
              + (String) ((MimeMultipart) m.getContent()).getBodyPart(1).getContent());
    }
    return msgs;
