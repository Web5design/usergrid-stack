...
public abstract class PrincipalIdentifier
{
    ...
    protected void grantAppRoles( UsergridAuthorizationInfo info, EntityManager em, UUID applicationId, TokenInfo token,
                                  Set<String> rolenames ) throws Exception
    {
        ...
        for ( String rolename : rolenames )
        {
            ...
            final String role =
                    new StringBuilder( "application-role:" ).append( applicationId ).append( ":" ).append( rolename )
                                                            .toString();
    ...
    protected void grant( UsergridAuthorizationInfo info, UUID applicationId, Set<String> permissions )
    {
        if ( permissions != null )
        {
            for ( String permission : permissions )
            {
                if ( isNotBlank( permission ) )
                {
                    ...
                    permission = new StringBuilder( "applications:" ).append( operations ).append( ":" )
     ...
     */
    public abstract void populateAuthorizatioInfo( UsergridAuthorizationInfo info, UsergridRealm realm )
            throws Exception;
}
