...
public class CassandraMQUtils {
  ...
  public static Map<ByteBuffer, ByteBuffer> serializeMessage(Message message) {
    ...
    for (Entry<String, Object> property : message.getProperties().entrySet()) {
      ...
      } else if (Message.MESSAGE_TYPE.equals(property.getKey()) || Message.MESSAGE_ID.equals(property.getKey())) {
  ...
  public static Message deserializeMessage(List<HColumn<String, ByteBuffer>> columns) {
    ...
    for (HColumn<String, ByteBuffer> column : columns) {
      if (Message.MESSAGE_TYPE.equals(column.getName()) || Message.MESSAGE_ID.equals(column.getName())) {
        properties.put(column.getName(), ConversionUtils.object(Message.MESSAGE_PROPERTIES.get(column.getName()), column.getValue()));
      } else {
  ...
  public static Map<ByteBuffer, ByteBuffer> serializeQueue(Queue queue) {
    ...
    for (Entry<String, Object> property : queue.getProperties().entrySet()) {
      ...
      if (Queue.QUEUE_ID.equals(property.getKey()) || Queue.QUEUE_NEWEST.equals(property.getKey())
          || Queue.QUEUE_OLDEST.equals(property.getKey())) {
        continue;
      }
  ...
  public static Queue deserializeQueue(List<HColumn<String, ByteBuffer>> columns) {
    ...
    Map<String, Object> properties = new HashMap<String, Object>();
    for (HColumn<String, ByteBuffer> column : columns) {
      if (Queue.QUEUE_PROPERTIES.containsKey(column.getName())) {
        properties.put(column.getName(), ConversionUtils.object(Queue.QUEUE_PROPERTIES.get(column.getName()), column.getValue()));
      } else {
        properties.put(column.getName(), JsonUtils.fromByteBuffer(column.getValue()));
      }
    }
  ...
  public static UUID getConsumerId(UUID queueId, QueueQuery query) {
    ...
    if (query.getPosition() == QueuePosition.CONSUMER) {
      consumerId = query.getConsumerId();
      if ((consumerId == null) && (query.getPosition() == QueuePosition.CONSUMER)) {
