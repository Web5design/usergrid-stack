...
public class CounterQuery {
	...
	public CounterQuery(CounterQuery q) {
		if (q != null) {
			...
			categories = q.categories != null ? new ArrayList<String>(
					q.categories) : null;
			counterFilters = q.counterFilters != null ? new ArrayList<Query.CounterFilterPredicate>(
	...
	public static CounterQuery fromQueryParams(Map<String, List<String>> params) {
		...
		List<Query.CounterFilterPredicate> counterFilters = null;
		...
		if (!isEmpty(l)) {
			counterFilters = Query.CounterFilterPredicate.fromList(l);
		}
	...
	public CounterQuery addCounterFilter(String counter) {
		Query.CounterFilterPredicate p = Query.CounterFilterPredicate.fromString(counter);
		if (p == null) {
			return this;
		}
		if (counterFilters == null) {
			counterFilters = new ArrayList<Query.CounterFilterPredicate>();
		}
	...
	public CounterQuery withCounterFilters(
			List<Query.CounterFilterPredicate> counterFilters) {
		this.counterFilters = counterFilters;
		return this;
	}
