...
import static org.usergrid.persistence.mq.Message.MESSAGE_PROPERTIES;
		...
		if (message.isIndexed()) {
			...
			for (Map.Entry<String, Object> property : message.getProperties()
				...
				if (!MESSAGE_PROPERTIES.containsKey(property.getKey())
						&& QueueIndexUpdate.validIndexableValueOrJson(property.getValue())) {
		...
		if (propertyEntryList != null) {
			for (Entry<String, List<Entry<String, Object>>> property : propertyEntryList
				...
				for (Map.Entry<String, Object> indexEntry : property.getValue()) {
					...
					if (QueueIndexUpdate.validIndexableValue(indexEntry.getValue())) {

						batch.addInsertion(
								bytebuffer(key(queueId, shard_ts,
										indexEntry.getKey())),
								QueuesCF.PROPERTY_INDEX.getColumnFamily(),
								createColumn(
										new DynamicComposite(
												QueueIndexUpdate.indexValueCode(indexEntry
                                                        .getValue()),
						...
						batch.addInsertion(
								bytebuffer(key(queueId,
										QueueManagerImpl.DICTIONARY_MESSAGE_INDEXES)),
				...
				batch.addInsertion(
						bytebuffer(key(queueId, QueueManagerImpl.DICTIONARY_MESSAGE_INDEXES)),
						QueuesCF.QUEUE_DICTIONARIES.getColumnFamily(),
