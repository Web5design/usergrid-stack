...
//  private static final String dictionaryName;
   ...
   * @param entityId The entityId to use.  Can be a source for outgoing connections, or target for incoming connections
   *  @param outgoing True if this is a search from source->target on the edge, false if it is a search from target<-source
   * @param pageSize The page size to use for batch fetching
   */
  public ConnectionTypesIterator(CassandraService cass, UUID applicationId, UUID entityId, boolean outgoing, int pageSize) {
    this.cass = cass;
    this.applicationId = applicationId;
    this.pageSize = pageSize;

    this.key = outgoing? key(entityId, DICTIONARY_CONNECTED_TYPES) : key(entityId, DICTIONARY_CONNECTING_TYPES);
  }
  ...
  public boolean load() throws Exception {
    ...
    List<HColumn<ByteBuffer, ByteBuffer>> results = cass.getColumns(cass.getApplicationKeyspace(applicationId),
        ENTITY_DICTIONARIES, key, start, null, selectSize, false);
