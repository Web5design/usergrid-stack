...
public abstract class GeoIndexSearcher {
  ...
  /**
   * Perform a search from the center. The corresponding entities returned must
   * be >= minDistance(inclusive) and < maxDistance (exclusive)
   * 
   * @param center
   * @param maxResults
  ...
  public final SearchResults proximitySearch(final EntityLocationRef minMatch, final List<String> geoCells,
      ...
      double minDistance, double maxDistance, final int maxResults) throws Exception {
  ...
  protected TreeSet<HColumn<ByteBuffer, ByteBuffer>> query(Object key, List<String> curGeocellsUnique,
      Point searchPoint, UUID startId,
      int count)
      throws Exception {
    ...
    TreeSet<HColumn<ByteBuffer, ByteBuffer>> columns = IndexMultiBucketSetLoader.load(cass, ENTITY_INDEX, appId, keys,
        start, null, count, false);
  ...
  protected abstract TreeSet<HColumn<ByteBuffer, ByteBuffer>> doSearch(List<String> geoCells, UUID startId,
      Point searchPoint,
      String propertyName, int pageSize)
      throws Exception;
