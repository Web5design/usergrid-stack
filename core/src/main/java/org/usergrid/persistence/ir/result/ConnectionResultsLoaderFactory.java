...
public class ConnectionResultsLoaderFactory implements ResultsLoaderFactory {
  ...
  public ResultsLoader getResultsLoader(EntityManager em, Query query, Results.Level level) {
    switch (level) {
      case IDS://Note that this is technically wrong.  However, to support backwards compatibility with the existing apis and usage, both ids and refs return a connection ref when dealing with connections
      case REFS:
